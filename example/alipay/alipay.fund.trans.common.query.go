...
func main() {
	...
	resp, err := client.AlipayFundTransCommonQuery(context.Background(), &types.FundTransCommonQuery{
		OutBizNo: "OUT_BIZ_NO_001",
	})
	if err != nil {
		log.Fatal(err)
	}
	log.Printf("Status: %s", resp.AlipayFundTransCommonQueryResponse.Status)
}
